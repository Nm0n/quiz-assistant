# bridge/quiz_ops.py
# QuizOpsMixin：作答、模式切换、收藏/错题切换、记忆模式、导航。
#
# 与拆分前 ControllerBridge 中对应方法一一对应，Slot 名、参数、返回值、
# 信号发射逻辑完全一致。仅做以下必要调整：
#
#   1. _refresh 通过 MRO 动态派发（由 ViewStateMixin 提供）。
#   2. 常量与 map_judgment 从原 main.py 顶部搬过来，import 路径不变。
...
class QuizOpsMixin(BridgeSignals):
    """模式 / 标签 / 记忆 / 作答 / 导航。"""
