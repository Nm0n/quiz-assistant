...
#
# 与拆分前的 ControllerBridge 中对应方法一一对应，方法名、Slot 名、
# 装饰器、参数、返回值、异常处理逻辑完全一致。仅做以下必要调整：
#
#   1. sys.platform == "android"  /  sys.platform != "android"
#      → is_android()  /  not is_android()
#      （PySide6 Android 环境下 sys.platform 可能是 "linux"，不可靠）
#
#   2. 原类内静态方法互引用 ControllerBridge._read_android_uri /
#      ControllerBridge._open_android_uri_for_write
#      → AndroidStorageMixin._read_android_uri /
#        AndroidStorageMixin._open_android_uri_for_write
#
#   3. _get_watch_dir 的桌面分支原以 main.py 所在目录为项目根，
#      本文件位于 bridge/ 子目录，因此加一级 dirname，
#      保证桌面数据目录仍是 <项目根>/data，行为不变。
#
#   4. loadJsonFile 内部转调 self.loadFromJson（FileOpsMixin 提供），
#      通过 Python 动态派发，Mixin 之间不直接 import。
...
class AndroidStorageMixin(BridgeSignals):
    """平台判断、存储权限、目录扫描、content:// URI 处理。"""
    ...
    def _get_watch_dir():
        ...
        注意：拆分前本函数在 main.py 中，__file__ 即 <项目根>/main.py，
              只需一次 dirname 即可得到项目根。拆分后本文件位于
              <项目根>/bridge/android_storage.py，需要两次 dirname。
        """
        if is_android():
    ...
    def loadJsonFile(self, path):
        # loadFromJson 由 FileOpsMixin 提供，通过 MRO 动态派发。
    ...
    def _resolve_read(raw):
        ...
        if raw.startswith("content://"):
            if not is_android():
                return None, None
            try:
                data = AndroidStorageMixin._read_android_uri(raw)
                if data is None:
                    return None, None
    ...
    def _resolve_write(raw):
        ...
        if raw.startswith("content://"):
            ...
            writer = None
            try:
                writer = AndroidStorageMixin._open_android_uri_for_write(raw)
                yield writer, raw
            except Exception as e:
